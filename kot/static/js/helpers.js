...
var calculate_entropy, decimal_to_hex, get_nice_size, get_offsets, hex, hexDigits, hex_to_decimal, max, min, nicefy, return_opposite, rgb2hex, rgb2hsv, status_notify;
...
calculate_entropy = function(pass) {
  var large_case, numbers, others, possible_symbols, small_case;
  possible_symbols = 0;
  numbers = /[0-9]/;
  small_case = /[a-z]/;
  large_case = /[A-Z]/;
  ...
  return Math.round(pass.length * Math.log(possible_symbols) / Math.log(2));
};
...
nicefy = function(number) {
  ...
  for (i = _i = 0, _ref = endings.length; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
    ...
    if ((endings[i].amount <= number && number < endings[i + 1].amount)) {
      return Math.round(number / endings[i].amount) + endings[i].ending;
    }
  }
};
